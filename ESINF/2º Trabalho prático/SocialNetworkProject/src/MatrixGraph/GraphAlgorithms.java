...
public class GraphAlgorithms {
    ...
    static <V,E> void DFS(AdjacencyMatrixGraph<V,E> graph, int index, LinkedList<V> verticesQueue) {
        ...
        Iterable<V> adjacentVertices = graph.directConnections(graph.vertices.get(index));
        
        for (V adjacentVertice : adjacentVertices) {
            int adjacentVerticeIndex = graph.toIndex(adjacentVertice);
            if (!verticesQueue.contains(graph.vertices.get(adjacentVerticeIndex))) {
                DFS(graph, adjacentVerticeIndex, verticesQueue);
            }
        }	
    }
