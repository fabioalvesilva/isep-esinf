...
public class AdjacencyMatrixGraph<V, E> implements BasicGraph<V, E>, Cloneable {
    ...
    */
    ...
    public Iterable<V> directConnections(V vertex) {  
        ...
        ArrayList<V> vertTemp = new ArrayList<>();
        int index = this.vertices.indexOf(vertex);
        int length = this.edgeMatrix[index].length;

        for (int i = 0; i < length; i++) {
            if (this.edgeMatrix[index][i] != null) {
                vertTemp.add(this.vertices.get(i));
            }
        }
        return vertTemp;
    }
    ...
    public Iterable<E> outgoingEdges(V vertex) {
        ...
        ArrayList<E> vertTemp = new ArrayList<>();
        ...
        for (int i = 0; i < length; i++) {
            if (this.edgeMatrix[index][i] != null) {
                vertTemp.add(this.edgeMatrix[index][i]);
            }
        }
        return vertTemp;
    }
